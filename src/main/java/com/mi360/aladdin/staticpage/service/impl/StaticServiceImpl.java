...
public class StaticServiceImpl implements IStaticService{
	...
	public void staticPagePC(String requestId, String sourceUrl, String fileName) {
		...
		String js = "<script type=\"text/javascript\">$.ajax({url:\"/login/islogin\",type:\"post\",dataType:\"json\",success:function(ret){if(ret.errcode==0){$(\".login_or_nickname\").find(\"a\").attr(\"href\",\"/user\").text(ret.nickname);$(\".register_or_logout\").find(\"a\").attr(\"href\",\"javascript:logout();\").text(\"退出\");$(\"body\").prepend(\"<input type='hidden' name='isLogin' value='1'/>\")}else{$(\"body\").prepend(\"<input type='hidden' name='isLogin' value='0'/>\")}}});function logout(){$.ajax({url:\"/user/logout\",type:\"post\",success:function(ret){location.href=\"/\"},error:function(){console.log(\"退出失败\")}})}$(function(){$(\".icon-search\").click(function(){$(\".search-form\").submit();});});</script>";
